...
- `models/`: PyTorch model code, including the full WiFlow model, CSI spatial encoder, axial attention encoder, spatial-temporal fuser, joint cross-attention decoder, legacy temporal encoder, legacy attention pooler, legacy skeleton-aware decoder, and shared COCO17 skeleton topology. The active single-frame model path is CSI feature concat -> spatial encoder with feature-wise antenna mixing, feature-wise stems, and time-frequency residual blocks -> axial encoder -> joint cross-attention decoder. For `action_env` sequence runs, per-frame spatial feature maps pass through the spatial-temporal fuser before the joint cross-attention decoder.
