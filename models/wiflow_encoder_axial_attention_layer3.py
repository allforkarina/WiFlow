...
class WiFlowEncoderAxialAttentionLayer3(nn.Module):
    ...
    def _prepare_temporal_attention_input(self, x: torch.Tensor) -> torch.Tensor:
        batch_size, channels, keypoints, temporal = x.shape
        return x.permute(0, 2, 3, 1).reshape(batch_size * keypoints, temporal, channels)
    ...
    def _prepare_keypoint_attention_input(self, x: torch.Tensor) -> torch.Tensor:
        batch_size, channels, keypoints, temporal = x.shape
        return x.permute(0, 3, 2, 1).reshape(batch_size * temporal, keypoints, channels)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self._restore_temporal_attention_output(
            temporal_output,
            batch_size,
            keypoints,
            temporal,
        ...
        keypoint_input = self._prepare_keypoint_attention_input(x)
        ...
        keypoint_output = self.keypoint_norm(keypoint_output + keypoint_input)
        return self._restore_keypoint_attention_output(
            keypoint_output,
            batch_size,
            keypoints,
            temporal,
