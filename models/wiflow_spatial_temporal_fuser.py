...
class WiFlowSpatialTemporalFuser(nn.Module):
    """Fuse [B, N, 256, 29, 10] into the middle [B, 256, 29, 10] feature map."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim != 5:
            raise ValueError("WiFlowSpatialTemporalFuser expects input shaped [B, N, 256, 29, 10]")
        ...
        fused = self.output_norm(attended + tokens)
        middle = fused[:, self.middle_index]
        return middle.reshape(batch_size, height, width, self.embedding_dim).permute(0, 3, 1, 2)
