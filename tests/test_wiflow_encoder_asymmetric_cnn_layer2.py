...
def test_wiflow_encoder_asymmetric_cnn_layer2_stage_shapes() -> None:
    ...
    assert reshaped.shape == (2, 1, 10, 340)
    assert stem_output.shape == (2, 8, 10, 340)
    assert resblock1_output.shape == (2, 16, 10, 170)
    assert resblock2_output.shape == (2, 32, 10, 85)
    assert resblock3_output.shape == (2, 64, 10, 17)
...
def test_wiflow_encoder_asymmetric_cnn_layer2_shortcuts_match_main_path() -> None:
    ...
    assert resblock1_main.shape == resblock1_shortcut.shape == (2, 16, 10, 170)
    assert resblock2_main.shape == resblock2_shortcut.shape == (2, 32, 10, 85)
    assert resblock3_main.shape == resblock3_shortcut.shape == (2, 64, 10, 17)


def test_wiflow_encoder_asymmetric_cnn_layer2_supports_single_item_batch() -> None:
    layer = WiFlowEncoderAsymmetricCNNLayer2()
    x = torch.randn(1, 340, 10)
